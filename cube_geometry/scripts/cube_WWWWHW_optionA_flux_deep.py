...
print(">>> cube_WWWWHW_optionA_flux_deep.py starting up...")

# ---------- PARAMETERS (3D WWWWHW + WHY SINK + FLUX TRACKING, DEEP RUN) ----------
L = 12               # cube size: L x L x L
THRESHOLD = 6        # topple when height >= 6

# Deeper run so the system reaches SOC and we see many avalanches
...
def plot_log_log_with_fit(avalanche_sizes: np.ndarray):
    ...
    print(f"Number of avalanches: {len(avalanages)}")
    ...
    if exponent is not None:
        print(f"Fitted power-law exponent (3D WHY-sink, flux DEEP): {exponent:.3f}")
    ...
    plt.title(f"3D WWWWHW Cube Avalanches (WHY sink + flux, deep run, L={L}, steps={MEASURE_STEPS})")
...
def main():
    print(">>> Entering main() [3D WWWWHW, WHY sink, flux tracking DEEP]", flush=True)
...
if __name__ == "__main__":
    print(">>> __main__ guard triggered [3D WWWWHW, WHY sink, flux DEEP]", flush=True)
    main()
