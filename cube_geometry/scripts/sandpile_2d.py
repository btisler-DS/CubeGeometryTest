...
def run_simulation():
    print("Burn-in phase (2D sandpile)...")
    for t in range(BURN_IN_STEPS):
        drop_grain()
        relax_once(record_avalanche=False)
        if (t + 1) % (BURN_IN_STEPS // 10) == 0:
            print(f"  Burn-in {t + 1}/{BURN_IN_STEPS}")

    print("Measurement phase (2D sandpile)...")
    avalanche_sizes = []
    for t in range(MEASURE_STEPS):
        ...
        if (t + 1) % (MEASURE_STEPS // 10) == 0:
            print(
                f"  Measure {t + 1}/{MEASURE_STEPS}, "
...
def plot_log_log(avalanche_sizes: np.ndarray):
    ...
    plt.title(f"2D Sandpile Avalanches (N={N}, steps={MEASURE_STEPS})")
