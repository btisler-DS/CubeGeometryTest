...
print(">>> cube_WWWWHW_optionA_flux_safe.py starting up...")

# ---------- PARAMETERS (3D WWWWHW + WHY SINK + FLUX, SAFE CONFIG) ----------
L = 12                # cube size
THRESHOLD = 6         # topple threshold

# Make each topple slightly dissipative (global loss), not amplifying
WHY_INFLOW_WEIGHT = 0.8   # toward WHY face
NORMAL_WEIGHT     = 1.0   # all other directions

# Moderate run so it finishes in reasonable time
BURN_IN_STEPS  = 5_000
...
def relax_once_3d(record: bool = False):
    ...
    This version is guaranteed to terminate because each topple is globally
    dissipative: sum of neighbor weights < THRESHOLD.
    """
    global face_flux
    ...
    safety_count = 0
    SAFETY_LIMIT = L * L * L * 50  # generous but finite
...
def plot_log_log_with_fit(avalanche_sizes: np.ndarray):
    ...
    if exponent is not None:
        print(f"Fitted power-law exponent (3D WHY-sink, flux SAFE): {exponent:.3f}")
    ...
    plt.title(f"3D WWWWHW Cube Avalanches (WHY sink + flux, SAFE, L={L})")
...
def main():
    print(">>> Entering main() [3D WWWWHW, WHY sink, flux SAFE]", flush=True)
...
if __name__ == "__main__":
    print(">>> __main__ guard triggered [3D WWWWHW, WHY sink, flux SAFE]", flush=True)
    main()
