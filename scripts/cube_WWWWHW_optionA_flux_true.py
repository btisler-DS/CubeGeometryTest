...
print(">>> cube_WWWWHW_optionA_flux_true.py starting up...")

# ---------- PARAMETERS (3D WWWWHW + WHY ATTRACTOR, NORMAL RUN) ----------
L = 12                # cube size
...
WHY_INFLOW_WEIGHT = 1.2   # weight for load entering WHY-direction neighbor
NORMAL_WEIGHT     = 1.0   # all other directions
# -----------------------------------------------------------------------
...
def relax_once_3d(record: bool = False):
    ...
    NOTE: With WHY_INFLOW_WEIGHT = 1.2 and NORMAL_WEIGHT = 1.0,
    each internal topple distributes 1.2 + 5*1.0 = 6.2 units of load outwards
    while removing 6, i.e. the rule is slightly supercritical in the interior.
    A safety cap is used to prevent pathological non-termination.
    """
    global face_flux
    ...
    while np.any(topple_mask):
        ...
        if safety_count > SAFETY_LIMIT:
            print("WARNING: Safety limit reached in relax_once_3d; "
                  "forcing termination of avalanche.")
            break
...
def plot_log_log_with_fit(avalanche_sizes: np.ndarray):
    ...
    if exponent is not None:
        print(f"Fitted power-law exponent (3D WHY-attractor, flux TRUE): {exponent:.3f}")
    ...
    plt.title(f"3D WWWWHW Cube Avalanches (WHY attractor + flux, L={L})")
...
def main():
    print(">>> Entering main() [3D WWWWHW, WHY attractor, flux TRUE]", flush=True)
...
if __name__ == "__main__":
    print(">>> __main__ guard triggered [3D WWWWHW, WHY attractor, flux TRUE]", flush=True)
    main()
